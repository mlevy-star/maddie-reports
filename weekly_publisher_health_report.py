...
def get_report_window(as_of: datetime.date = None):
    ...
    current = today-7 through yesterday (today-1)
    prior   = today-14 through today-8
    Works correctly regardless of what day of the week the report runs.
    """
    if as_of is None:
        as_of = datetime.date.today()
    current_end = as_of - datetime.timedelta(days=1)    # yesterday
    current_start = as_of - datetime.timedelta(days=7)  # today minus 7
    prior_end = as_of - datetime.timedelta(days=8)      # today minus 8
    prior_start = as_of - datetime.timedelta(days=14)   # today minus 14
